...
public class MapTask extends MapReduceTask {
	...
	public MapTask(String configFileName, Reporter reporter, POA poa, Task task, Sorter sorter) throws IOException {
...
//			}
	...
	public void run() throws IOMapReduceException { 
		try {
			...
			{
				mapper.map(key.value, value.value, collector, reporter, sorter);
			}
			collector.flush();
			collector.close();                        
			input.close();
