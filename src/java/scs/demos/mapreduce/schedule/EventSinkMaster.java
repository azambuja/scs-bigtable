...
public class EventSinkMaster extends EventSinkConsumerServant {
	...
	private Task[] doLastTask (Phase status) {
		try{ 
				...
				master.setCurrentPhase(status);
				  ...
				  TaskServant(TaskStatus.END, null, 0)));	
	...
	private Task[] doReduce()  throws Exception {
	    	try {
                	...
                	for(int i=0; i<numReducers; i++) {
                                FileSplit[] inputSplit = new FileSplit[inputToReducers[i].size()];
                                inputToReducers[i].toArray(inputSplit);

                                TaskServant rs= new TaskServant(TaskStatus.REDUCE, inputSplit, i);                                 
	...
	protected void handleEvent(Any ev) {
		...
		synchronized(master){
			...
			//IComponent workerComponent = null;
			Worker worker = null;
                        Task task = null;         
			Task[] t = null;
			...
			while (enu.hasMoreElements()) {
				...
				if(id == taskId) {
					// TODO ERRO AQUI!!!
					// continuar
					/*workerComponent = master.getWorkingOn().remove(task);
					master.addWorkerQueue(workerComponent);	      
